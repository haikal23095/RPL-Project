...
// Gunakan id_user dari session untuk mengambil data admin yang login
// Asumsi $_SESSION["admin"] menyimpan NAMA admin.
$admin_nama = $_SESSION["admin"];
$query = $kon->prepare("SELECT * FROM user WHERE nama = ?");
$query->bind_param("s", $admin_nama);
$query->execute();
$result = $query->get_result();

if (!$result || $result->num_rows == 0) {
    $_SESSION['error'] = "Data profil admin tidak ditemukan.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    // Pastikan ini adalah path yang siap digunakan, e.g., '../uploads/namafile.jpg'
    $current_foto_db_path = $admin_data['foto']; 
    ...
    if (isset($_FILES['foto_profil']) && $_FILES['foto_profil']['error'] === UPLOAD_ERR_OK) {
        ...
        // Validasi dasar file (opsional, tapi sangat disarankan)
        ...
        if (move_uploaded_file($file_tmp, $target_file_absolute)) {
            ...
            // Path yang disimpan harus relatif dari root project untuk konsistensi
    ...
    if ($stmt_update->execute()) {
        ...
        // Perbarui $_SESSION["admin"] jika nama berubah
        if ($_SESSION["admin"] !== $nama) {
            $_SESSION["admin"] = $nama;
        }
        // Perbarui $admin_data untuk tampilan di halaman saat ini
        $admin_data['nama'] = $nama;
        $admin_data['foto'] = $new_foto_db_path; // Update foto juga
    } else {
...
if (isset($_POST['delete_profile_photo'])) {
    $id_user = $admin_data['id_user'];
    $current_foto_path = $admin_data['foto']; // Path dari database
        ...
        if ($absolute_delete_path && file_exists($absolute_delete_path) && is_writable($absolute_delete_path)) {
            if (unlink($absolute_delete_path)) {
                ...
                if ($stmt_delete_photo->execute()) {
                    ...
                    $admin_data['foto'] = ''; // Update data di halaman
                } else {
  ...
  <link href="../assets/img/LOGOCASALUXE2.png" rel="icon">
    ...
    <style>
        ...
        .btn-primary {
            ...
            color: #fff;
            margin: 0;
            margin-top: 0; /* Jarak atas dari bagian sebelumnya */
        }
        .btn-primary:hover {
            background-color: #167a70; /* Sedikit lebih gelap saat hover */
            border-color: #167a70;
...
<body>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-7 profile-info-section">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control info-value" id="nama_lengkap" name="nama" value="<?= htmlspecialchars($admin_data['nama']) ?>" required>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="info-row">
                                    ...
                                    <input type="text" class="form-control info-value-inline" id="email_display" value="<?= substr(htmlspecialchars($admin_data['email']), 0, 1) . '*********' . substr(htmlspecialchars($admin_data['email']), -8) ?>" disabled>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="info-row">
                                    ...
                                    <input type="text" class="form-control info-value-inline" id="no_telepon_display" value="<?= substr(htmlspecialchars($admin_data['no_tlp']), 0, 2) . '*********' . substr(htmlspecialchars($admin_data['no_tlp']), -2) ?>" disabled>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="info-row">
                                    <textarea class="form-control info-value-inline" id="alamat_display" rows="3" disabled><?= htmlspecialchars($admin_data['alamat']) ?></textarea>
                        ...
                        <div class="col-md-5 profile-image-section">
                            <!-- PATH GAMBAR TELAH DIPERBAIKI -->
                            <img src="<?= !empty($admin_data['foto']) ? htmlspecialchars($admin_data['foto']) : 'https://placehold.co/150x150/cccccc/333333?text=No+Photo'; ?>" alt="Profile Image" class="profile-img">
    ...
    <!-- Modals for "Ganti" actions - These will submit to update_profile_field.php -->

    <!-- Modal Edit Email -->
    <div class="modal fade" id="editEmailModal" tabindex="-1" aria-labelledby="editEmailModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="update_profile_field.php" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="email" class="form-control" id="new_email" name="value" value="<?= htmlspecialchars($admin_data['email']) ?>" required>
                        </div>
                    </div>
                ...
                <form action="update_profile_field.php" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="new_phone" name="value" value="<?= htmlspecialchars($admin_data['no_tlp']) ?>" required>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                ...
                <form action="update_profile_field.php" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="new_address" name="value" rows="3" required><?= htmlspecialchars($admin_data['alamat']) ?></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
