...
<body>
                        ...
                        <?php if (count($pesanan) > 0): // Changed $pesanan_list to $pesanan ?>
                            <?php foreach ($pesanan as $pesanan_item): ?>
                                ...
                                <div class="card pesanan-card mb-4">
                                    <div
                                        class="card-header pesanan-header d-flex justify-content-between align-items-center flex-wrap">
                                        <div class="my-1">
                                            <strong>ID Pesanan:</strong> #<?= htmlspecialchars($pesanan_item['id_pesanan']) ?>
                                        </div>
                                        <span
                                            class="badge bg-primary my-1"><?= htmlspecialchars($pesanan_item['status_pesanan']) ?></span>
                                    </div>

                                    <div class="product-gallery px-3 py-2 d-flex flex-wrap gap-2">
                                        <?php if (!empty($produk_images)): ?>
                                            <?php foreach ($produk_images as $img): ?>
                                                <img src="../uploads/<?= htmlspecialchars($img['gambar']) ?>"
                                                    class="product-img-gallery" title="<?= htmlspecialchars($img['nama_produk']) ?>">
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                    ...
                                    <div class="card-footer-actions">
                                        <div class="row align-items-center">
                                            <div class="col-md-6 mb-2 mb-md-0">
                                                ...
                                                <h5 class="total-harga d-inline-block ms-2 mb-0">
                                                    <?= formatCurrency($pesanan_item['total_harga']) ?>
                                                </h5>
                                            </div>
                                            <div class="col-md-6 text-md-end">
                                                <button class="btn btn-outline-primary btn-sm btn-detail" type="button"
                                                ...
                                                <?php if ($pesanan_item['status_pesanan'] === 'Selesai'): ?>
                                                    <?php if (empty($pesanan_item['sudah_dinilai'])): ?>
                                                        <button type="button" class="btn btn-primary btn-sm open-review-modal"
                                                            data-order-id="<?= $pesanan_item['id_pesanan'] ?>">Nilai</button>
                                                    <?php else: ?>
                                                        <button class="btn btn-success btn-sm" disabled>Dinilai</button>
                                                    <?php endif; ?>
                                                    <a href="checkout.php?ulang=<?= $pesanan_item['id_pesanan'] ?>"
                                                        class="btn btn-success btn-sm">Beli Lagi</a>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="modal fade" id="reviewModal" tabindex="-1">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Beri Ulasan untuk Pesanan #<span id="modal-order-id"></span></h5><button
                        type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div id="modal-alert-placeholder"></div>
                    <form id="reviewForm"><input type="hidden" id="form-order-id" name="order_id">
                        <div id="review-product-list">
                            <div class="text-center" id="modal-loader">
                                <div class="spinner-border"><span>Loading...</span></div>
                            </div>
                        </div>
                    </form>
                </div>
                <div class="modal-footer"><button type="button" class="btn btn-secondary"
                        data-bs-dismiss="modal">Tutup</button><button type="button" class="btn btn-primary"
                        id="submit-review-btn">Kirim Ulasan</button></div>
            </div>
        </div>
    </div>
